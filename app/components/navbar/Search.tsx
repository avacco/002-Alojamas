...
const Search = () => {
  ...
  // Obtiene valores de parametros en la url
  const locationValue = params?.get("locationValue");
  const startDate = params?.get("startDatee");
  const endDate = params?.get("endDate");
  const guestCount = params?.get("guestCount");


  /* Cambia los valores de los label en la barra de busqueda segun los valores antes definidos */
  ...
  const guestLabel = useMemo(() => {
    if(guestCount) return `${guestCount} personas`;
